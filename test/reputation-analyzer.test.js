...
describe("agents/reputation-analyzer", function () {
    ...
    describe("calculateTrend", function () {
        it("Should return 'new' for < 2 records", function () {
            expect(analyzer.calculateTrend([])).to.equal("new");
            expect(analyzer.calculateTrend([{ rating: 80 }])).to.equal("new");
        });

        it("Should return 'new' when no older records to compare", function () {
            const rec = [{ rating: 80 }, { rating: 85 }];
            expect(analyzer.calculateTrend(rec)).to.equal("new");
        });

        it("Should return 'improving' when recent avg > older avg + 10", function () {
            const rec = [
                { rating: 50 }, { rating: 50 }, { rating: 50 },
                { rating: 80 }, { rating: 80 }, { rating: 80 },
            ];
            expect(analyzer.calculateTrend(rec)).to.equal("improving");
        });

        it("Should return 'declining' when recent avg < older avg - 10", function () {
            const rec = [
                { rating: 90 }, { rating: 90 }, { rating: 90 },
                { rating: 50 }, { rating: 50 }, { rating: 50 },
            ];
            expect(analyzer.calculateTrend(rec)).to.equal("declining");
        });
        ...
        it("Should return 'stable' when within ±10", function () {
            ...
            expect(analyzer.calculateTrend(rec)).to.equal("stable");
        });
    });
