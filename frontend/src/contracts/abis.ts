...
export function getSigner(): ethers.Wallet {
  ...
  return new ethers.Wallet(HARDHAT_PRIVATE_KEYS[0], provider);
}

export function getRegisterSigner(index: number): ethers.Wallet {
  const provider = getProvider();
  const key = HARDHAT_PRIVATE_KEYS[index % HARDHAT_PRIVATE_KEYS.length];
  return new ethers.Wallet(key, provider);
}

export const HARDHAT_PRIVATE_KEY = HARDHAT_PRIVATE_KEYS[0];
