/**
 * API Server 集成测试（supertest）
 *
 * 测试策略：直接 require api-server.js 拿到 app 实例，用 supertest 注入。
 * api-server.js 启动时会调 app.listen，但 supertest 会自动注入 ephemeral port，
 * 不会真正占用固定端口。
 */
...
// 在 require api-server 之前设置必要环境变量
process.env.SILICONFLOW_API_KEY = "test-key";
process.env.ROUTER_SIGNER_PRIVATE_KEY = "0x59c6995e998f97a5a0044966f0945389dc9e86dae88c7a8412f4603b6b78690d";
process.env.REPUTATION_SIGNER_PRIVATE_KEY = "0x5de4111afa1a4b94908f83103eb1f1706367c2e68ca870fc3fb9a804cdab365a";
process.env.WORKER_DEMO_PRIVATE_KEY = "0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80";
process.env.AGENT_DID_ADDRESS = "0x5FbDB2315678afecb367f032d93F642f64180aa3";
process.env.AUDIT_LOG_ADDRESS = "0xe7f1725E7734CE288F8367e1Bb143E90bb3F0512";
process.env.REPUTATION_ADDRESS = "0x9fE46736679d2D9a65F0992F2272dE9f3c7fa6e0";
process.env.API_PORT = "0";  // 随机端口
process.env.API_ACCESS_KEYS = "";  // 默认无认证
...
describe("API Server (集成测试)", function () {
    ...
    before(async function () {
        // 替换 LLM/合约调用以避免真实依赖
        sinon.stub(require("axios"), "post").resolves({
            data: {
        ...
        app = require("../agents/api-server");
    });

    after(function () {
        sinon.restore();
        // 强制结束进程，防止 ethers providers 重试导致进程挂起
    ...
    describe("GET /api/health", function () {
        it("Should return 200 with health info", async function () {
            ...
            // routerSigner 在 init 时为 undefined（NonceManager 包装未异步解析）
            // apiAuthRequired 应是 boolean
    ...
    describe("GET /api/reputation/summary", function () {
        it("Should return summary object", async function () {
            ...
            // 可能 200（成功）或 500（合约调用失败）；我们只检查不 crash
